...
def generate_synthetic_dataset(num_images, backgrounds_path, transparent_reference_path, output_image_dir, previous_csv_path, output_csv_path, image_size, noise=True, augment=False):
    ...
    Args:
        ...
        augment (bool, optional): If it is an augmentation of the dataset -> we have already existing images and we want to add the synthetic ones. Defaults to False. 
    """        
    ...
    with open(output_csv_path, "w", newline="") as csvfile:
        ...
        for i, background_path in enumerate(backgrounds):
            ...
            # If in augment mode, we check the already existing label for the image (considered as a background) from the previous csv file
            if augment:
                # get the name of the background image
                background_name = background_path.stem
                # remove the L from the name if presetn
                if background_name.startswith("L"):
                    background_name = background_name[1:]
                    
                # check if the background image is in the previous csv file
                with open(previous_csv_path, "r") as prev_csvfile:
                    
                    prev_reader = csv.reader(prev_csvfile)
                    prev_header = next(prev_reader)
                    
                    # check if the background image is in the previous csv file
                    for row in prev_reader:
                        if row[0] == background_name:
                            # get the label for this background image
                            old_label = {prev_header[i]: int(row[i]) for i in range(1, len(prev_header))}
                            break
                    
                    # If the old label was not found, we throw an error
                    if not old_label:
                        raise ValueError(f"Background image {background_name} not found in the previous CSV file.")
            ...
            for j in tqdm(range(n_image)):
                ...
                # add the old label to the new label
                for key in label.keys():
                    if key in old_label:
                        label[key] += old_label[key]
